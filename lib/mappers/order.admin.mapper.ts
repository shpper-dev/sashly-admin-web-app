...
export function mapOrderData(id: string, data: DocumentData | undefined): Order {
  if (!data) {
    throw new Error(`Order document ${id} is empty`);
  }
...
/**
 * Client-side convenience wrapper — unwraps a browser QueryDocumentSnapshot
 * and delegates to mapOrderData. All existing call sites (e.g.
 * `snap.docs.map(mapOrder)` in metrics-orders.ts) keep working unchanged.
 */
// export function mapOrder(doc: QueryDocumentSnapshot<DocumentData>): Order {
//   return mapOrderData(doc.id, doc.data());
// }