      ...
      <DrawerContent className="max-w-140! h-full w-[40vw]  right-0 left-auto mt-0 rounded-none">
        <div className="flex flex-col h-full">
          ...
          <DrawerHeader className="flex flex-row items-center justify-between border-b pb-4">
            ...
            <div>
              <DrawerTitle className="text-lg font-semibold uppercase">
            ...
            <div className="flex items-center justify-between bg-slate-50 rounded-xl p-4">
              <div className="flex gap-2 items-center">
                <div>
                  ...
                  <p className={`font-semibold uppercase ${order.latestStatus.status != "cancelled" ? "text-purple-600": "text-red-600"}`}>
                ...
                ? new Date(order.expectedDeliveryTime).toLocaleDateString("en-GB", { day: "2-digit", month: "2-digit", year: "2-digit" })
                : "—"}
                  </p>
                </div>
              </div>
             ...
             onSuccess={() => onStatusUpdate(order.id)}>
               <button className="px-5 py-2 bg-[#02D0FF] hover:bg-[#10ccf7] text-white rounded-full text-sm cursor-pointer">
                UPDATE STATUS
               </button>
              </UpdateOrderDialog> 
            </div>
            ...
            {order.items.map((item, i) => (
              ...
              >
                <div>
                  ...
                  <p className="font-semibold text-slate-800">
                    {item.serviceName} | {item.serviceArabicName}
                  </p>
                  <p className="text-xs text-slate-400">
                    UNIT PRICE: SAR {item.servicePrice.toFixed(2)}
                  </p>
                </div>

                <div className="text-right">
                  <p className="text-xs text-slate-400">x{item.count}</p>
                  <p className="font-semibold">SAR {(item.servicePrice * item.count).toFixed(2)}</p>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="border-t p-6 space-y-4">
            <div className="flex justify-between ">
              ...
              <span className="text-ms font-bold">SAR {order.totalPrice.toFixed(2)}</span>
            </div>
